...
//!
//! This module creates `~/.jao/` if needed, reads `config.toml`, applies
//! defaults and normalization, and optionally loads the trust manifest.

use std::fs;

use home::home_dir;

use crate::config::models::{CURRENT_CONFIG_VERSION, JaoConfig, JaoConfigFile, JaoContext};
#[cfg(feature = "trust-manifest")]
use crate::trust;
use crate::{JaoError, JaoResult, config};
...
///
/// When `trust-manifest` is enabled this also normalizes the trustfile path,
/// updates the config on disk if normalization changed anything, and loads the
/// trust manifest into the returned [`JaoContext`].
pub(crate) fn load_or_init() -> JaoResult<JaoContext> {
